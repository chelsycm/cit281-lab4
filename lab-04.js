// Require the Fastify framework and instantiate it
const fastify = require("fastify")();
// Handle GET verb for / route using Fastify
// Note use of "chain" dot notation syntax
fastify.get("/", (request, reply) => {
  reply
    .code(200)
    .header("Content-Type", "text/html; charset=utf-8")
    .send("<h1>Hello from Lab 4!</h1>");
});

// Route for /name 
fastify.get("/name", (request, reply) => {

  // (A) Get information from the request
  const {first,last} = request.query;
  
  //(B) Modify/Transform/Use information from the request in some way
  //<variable> = <conditional statement> ? <variable value if conditional is true> : <variable if conditional is fale>
  const name = first && last ? `${first} ${last}` : `Guest`;

  // (C) Return the results of our modification/transformation to the client
  reply
    .code(200)
    .header("Content-Type", "text/html; charset=utf-8")
    .send(`<h1>Hello, ${name}</h1>`);
});
// Start server and listen to requests using Fastify
const listenIP = "localhost";
const listenPort = 8080;
fastify.listen(listenPort, listenIP, (err, address) => {
  if (err) {
    console.log(err);
    process.exit(1);
  }
  console.log(`Server listening on ${address}`);
});